...
def _indexer_with_32bit_outputs(indexer):
  class Indexer:
    def __getitem__(self, *args):
      result = indexer.__getitem__(*args)
      return tree_util.tree_map(lambda x: x.astype(dtypes._dtype_to_32bit_dtype[x.dtype]), result)
  return Indexer()
...
class LaxBackedNumpyTests(jtu.JaxTestCase):
  ...
  def testArgMinMaxEmpty(self, name, np_op, jnp_op):
    ...
    def np_fun(*args):
      return np_op(*args, axis=0).astype(jnp.int_)
  ...
  def testSetdiff1d(self, shape1, shape2, dtype1, dtype2):
    ...
    self._CheckAgainstNumpy(np.setdiff1d, jnp.setdiff1d, args_maker, check_dtypes=False)
  ...
  def testPoly(self, a_shape, dtype, rank):
    ...
    rng = jtu.rand_default(self.rng())
    # TODO(jakevdp[x64]): this change is because promote_dtypes_inexact(int64) results in float32.
    # Should it result in float64 instead?
  ...
  def testDiagIndices(self, ndim, n):
    np.testing.assert_equal(tuple(ind.astype(jnp.int_) for ind in np.diag_indices(n, ndim)),
  ...
  def testDiagIndicesFrom(self, dtype, shape):
    rng = jtu.rand_default(self.rng())
    np_fun = lambda *args: tuple(out.astype(jnp.int_) for out in np.diag_indices_from(*args))
  ...
  def testFrexp(self, shape, dtype, rng_factory):
    ...
    if (dtype not in [jnp.bfloat16, np.float16, np.float32]
      ...
      self.skipTest("Only run float64 testcase when float64 is enabled.")
    rng = rng_factory(self.rng())
    np_fun = _promote_like_jnp(np.frexp)
    jnp_fun = jnp.frexp
    args_maker = lambda: [rng(shape, dtype)]
    self._CheckAgainstNumpy(np_fun, jnp_fun, args_maker, check_dtypes=False)
    self._CompileAndCheck(jnp_fun, args_maker)
  ...
  def testAstypeNone(self):
    ...
    np_op = jtu.with_32bit_outputs(lambda x: np.asarray(x).astype(None))
  ...
  def testArgsort(self, dtype, shape, axis):
    ...
    if axis is not None:
      jnp_fun = partial(jnp_fun, axis=axis)
      np_fun = partial(np_fun, axis=axis)
    np_fun = jtu.with_32bit_outputs(np_fun)
    self._CheckAgainstNumpy(np_fun, jnp_fun, args_maker)
    self._CompileAndCheck(jnp_fun, args_maker)
  ...
  @parameterized.named_parameters(jtu.cases_from_list(
      ...
      for pytype, dtype in [(int, jnp.int64), (float, jnp.float64),
                            (bool, jnp.bool_), (complex, jnp.complex128)]
  ...
  def testMgrid(self):
    np_mgrid = _indexer_with_32bit_outputs(np.mgrid)
  ...
  def testOgrid(self):
    np_ogrid = _indexer_with_32bit_outputs(np.ogrid)
  ...
  def testR_(self):
    ...
    self.assertArraysEqual(np.r_['0,2', [1,2,3], [4,5,6]].astype(jnp.int_),
                           jnp.r_['0,2', [1,2,3], [4,5,6]])
    self.assertArraysEqual(np.r_['0,2,0', [1,2,3], [4,5,6]].astype(jnp.int_),
                           jnp.r_['0,2,0', [1,2,3], [4,5,6]])
    self.assertArraysEqual(np.r_['1,2,0', [1,2,3], [4,5,6]].astype(jnp.int_),
                           jnp.r_['1,2,0', [1,2,3], [4,5,6]])
    # negative 1d axis start
    self.assertArraysEqual(np.r_['0,4,-1', [1,2,3], [4,5,6]].astype(jnp.int_),
                           jnp.r_['0,4,-1', [1,2,3], [4,5,6]])
    self.assertArraysEqual(np.r_['0,4,-2', [1,2,3], [4,5,6]].astype(jnp.int_),
                           jnp.r_['0,4,-2', [1,2,3], [4,5,6]])

    # matrix directives
    with warnings.catch_warnings():
      warnings.filterwarnings("ignore", category=PendingDeprecationWarning)
      self.assertArraysEqual(np.r_['r',[1,2,3], [4,5,6]].astype(jnp.int_),
                             jnp.r_['r',[1,2,3], [4,5,6]])
      self.assertArraysEqual(np.r_['c', [1, 2, 3], [4, 5, 6]].astype(jnp.int_),
    ...
    atol = 1e-6
    rtol = 1e-6
    self.assertAllClose(np.r_[-1:1:6j].astype(jnp.float_),
                        jnp.r_[-1:1:6j],
                        atol=atol,
                        rtol=rtol)
    self.assertAllClose(np.r_[-1:1:6j, [0]*3, 5, 6].astype(jnp.float_),
                        jnp.r_[-1:1:6j, [0]*3, 5, 6],
                        atol=atol,
                        rtol=rtol)
    # Non-integer steps
    self.assertAllClose(np.r_[1.2:4.8:0.24].astype(jnp.float_),
                        jnp.r_[1.2:4.8:0.24],
                        atol=atol,
                        rtol=rtol)
  ...
  def testC_(self):
    ...
    self.assertArraysEqual(np.c_['0,2', [1,2,3], [4,5,6]].astype(jnp.int_), jnp.c_['0,2', [1,2,3], [4,5,6]])
    self.assertArraysEqual(np.c_['0,2,0', [1,2,3], [4,5,6]].astype(jnp.int_), jnp.c_['0,2,0', [1,2,3], [4,5,6]])
    self.assertArraysEqual(np.c_['1,2,0', [1,2,3], [4,5,6]].astype(jnp.int_), jnp.c_['1,2,0', [1,2,3], [4,5,6]])
    # negative 1d axis start
    self.assertArraysEqual(np.c_['0,4,-1', [1,2,3], [4,5,6]].astype(jnp.int_), jnp.c_['0,4,-1', [1,2,3], [4,5,6]])
    self.assertArraysEqual(np.c_['0,4,-2', [1,2,3], [4,5,6]].astype(jnp.int_), jnp.c_['0,4,-2', [1,2,3], [4,5,6]])
    # matrix directives, avoid numpy deprecation warning
    with warnings.catch_warnings():
      warnings.filterwarnings("ignore", category=PendingDeprecationWarning)
      self.assertArraysEqual(np.c_['r',[1,2,3], [4,5,6]].astype(jnp.int_), jnp.c_['r',[1,2,3], [4,5,6]])
      self.assertArraysEqual(np.c_['c', [1, 2, 3], [4, 5, 6]].astype(jnp.int_), jnp.c_['c', [1, 2, 3], [4, 5, 6]])
    ...
    atol = 1e-6
    rtol = 1e-6
    self.assertAllClose(np.c_[-1:1:6j].astype(jnp.float_),
                        jnp.c_[-1:1:6j],
                        atol=atol,
                        rtol=rtol)

    # Non-integer steps
    self.assertAllClose(np.c_[1.2:4.8:0.24].astype(jnp.float_),
                        jnp.c_[1.2:4.8:0.24],
                        atol=atol,
                        rtol=rtol)
  ...
  @parameterized.named_parameters(jtu.cases_from_list(
    {"testcase_name": f"_dtype={str(dtype)}", "dtype": dtype}
  ...
  def testArange64Bit(self, dtype):
    ...
    np_fun = lambda: np.arange(*args, dtype=dtype or jnp.float_)
    args_maker = lambda: []
    self._CheckAgainstNumpy(np_fun, jnp_fun, args_maker)
    self._CompileAndCheck(jnp_fun, args_maker)
